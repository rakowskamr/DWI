...
cd(dir_ppnt)
session_name=dir('*_SOLID_CHARMED_IRLLS');%fr
%session_name = dir('*_SOLID_DTInonlinear_maxb1500');%md/fa
for K = 1 : length(session_name)
    fname = session_name(K).name;
    name    = sprintf('S%d_SOLID_CHARMED_IRLLS',session(ss)); %Fr
    %name    =
    %sprintf('S%d_SOLID_DTInonlinear_maxb1500',session(ss));%MD/FA
...
%fsource = fullfile(source,filename);

%fsourceFA = filenameFA;
%fsourceMD = filenameMD;
fsourceFr = filenameFr;

%fdestination = fullfile(destination,newfilename);
